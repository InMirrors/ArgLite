...
为了减少在添加带值短选项时出现遗漏的可能性，建议使用下面的 RegEx 查找所有的带值短选项。在支持多光标的编辑器 / IDE 中通常可以用它选中所有带值短选项。因为不更新 `setShortNonFlagOptsStr()` 也能使用常规的、用空格分隔的语法 `-n 123`，所以添加选项后完全可以先不修改字符串，等当前阶段的工作结束后，例如要提交时，再用下面的 RegEx 选中，然后一次性更新多个选项。
