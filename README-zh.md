...
- [argparse](https://github.com/morrisfranken/argparse) 足够轻量，接口易用，实现的方式很巧妙，是我最满意的库。不过帮助的格式和命令行参数的语法和惯例不太一样，例如不支持短选项后直接带值 (`-n123`), 不支持多次传递同一个选项以传递多个值 (`-lmylib1 -lmylib2`)。
