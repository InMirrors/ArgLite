...
| Name         | Time (s) | Size (KB) |
|--------------|----------|-----------|
| ArgLite Mini |     0.92 |     110.2 |
| ArgLite Full |     1.00 |     134.2 |
| CLI11        |     1.36 |     550.3 |
| cxxopts      |     1.82 |     278.3 |
| args         |     1.51 |     278.3 |
| argparse     |     1.18 |     130.3 |
...
| Name         | Time (s) | Size (KB) |
|--------------|----------|-----------|
| ArgLite Mini |     1.25 |      54.3 |
| ArgLite Full |     1.48 |      62.3 |
| CLI11        |     1.63 |     498.3 |
| cxxopts      |     2.52 |     118.3 |
| args         |     2.76 |     150.3 |
| argparse     |     1.66 |      74.3 |

可见 ArgLite 确实是最轻量的，精简版的优势特别明显。 argparse 的表现也很好，只是稍差于 ArgLite 完整版，属于同一个水平。 CLI11 得益于链接了现成的库文件，编译时间较短（还是比不上 ArgLite），但二进制体积最大。 cxxopts, args 的表现一般般，只是比全能库 CLI11 轻而已，而且因为不能链接现成的库文件，编译时间垫底。

所以 ArgLite 确实在轻量方面表现最好，而且提供了易用的接口和美观的输出。 argparse 在轻量和易用方面也做得好，只是它的输出不够友好。 CLI11 也有易用的接口，但它是个全能库，对于简单和中等程序来说太重了。其他两个库无论是轻量还是接口易用性都表现得不好，比不上其他库。特别是 cxxopts, 禁用 `<regex>` 后表现仍是一般，不禁用的话结果会翻倍。
