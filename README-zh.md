...
为了减少在添加带值短选项时出现遗漏的可能性，建议使用下面的 RegEx 查找所有的带值短选项：
...
在支持多光标的编辑器 / IDE 中通常可以用它们选中所有带值短选项。因为不更新 `setShortNonFlagOptsStr()` 也能使用常规的、用空格分隔的语法 (`-n 123`)，所以不必添加一个选项就更新一次。建议等当前阶段的工作结束后，例如要提交时，再用提供 RegEx 批量选中，然后一次性更新多个选项。也可以更进一步，使用脚本自动化地完成这个修改。
