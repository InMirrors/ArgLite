...
- [args](https://github.com/Taywee/args) 轻量，但接口不好用。
- [argh](https://github.com/adishavit/argh) 超级轻量但太简陋了，连自动生成帮助都没有，所以也不好用。
- [argparse](https://github.com/morrisfranken/argparse) 很轻量，接口易用，实现的方式很巧妙，是我最满意的库。不过帮助的格式和命令行参数的语法和惯例不太一样，例如不支持短选项后直接带值 (`-n123`), 不支持多次传递同一个选项以传递多个值 (`-lmylib1 -lmylib2`)。
...
本项目提供了封装良好的示例，使得你不用关心太多库的具体用法也能通过改写示例完成命令行参数解析任务。如果你想快速上手，推荐参考 [simple_wrapped.cpp](./examples/simple_wrapped.cpp)。详见[示例](#-示例)章节。

这里介绍精简版的用法。两个版本的接口相似度很高，可以轻松迁移。本例只是展示基本用法，这些接口的具体用法请查阅[接口说明](#-接口说明)章节。
...
**和其他同类库最大的区别**: 不像其他库需要先注册完全部选项后才能解析然后取值，本库注册选项的同时就能获取值。上面的 `hasFlag` 和 `getXxx` 和其他库的 `add_option` 类似，都是提供参数的相关信息，但它们会同时返回解析结果。我们将这个模式称为 **注册即获取**。
...
和主流库相比，本库的一大优势是可以做到一条语句完成一个参数，多加一个参数只要多加一条语句。例如 CLI11 加一个参数要加一条声明语句，提供用于后续绑定的变量，然后添加一条注册语句。 cxxopts 则是要一条注册语句，解析后还要一条取值语句。其他库大部分都是这两种情况之一，都有同一个参数的代码不够集中的问题，只是具体的操作不同。
...
如果你只是想使用这个库，不想了解它的细节，你可以忽视这个小节。本项目提供了具有良好封装的示例，你完全可以复制粘贴示例代码后修改。详见[示例](#-示例)章节。
...
几乎没有 C++ 命令行参数解析库会考虑这个功能，导致 C++ 写的 CLI 程序的帮助不够美观、不够现代。实际上这个功能的基础版很容易实现，本库的精简版都能够支持。只是因为采用了简单的实现，在旧的 Windows 终端上会直接打印序列而不是格式化的文本。所以如果你要构建用于旧 Windows 平台的的应用，不启用这个功能可能会更好。
...
所有外部接口都提供了 Doxygen 注释。文档和注释是互补的关系，文档可能会省略一些注释的内容。如果你看了文档还是不懂怎么使用某个接口，请查阅注释。

如果你不需要使用子命令，所有接口都在 `ArgLite::Parser` 类中以静态函数的形式提供。完整版还提供了 `ArgLite::SubParser` 类用于子命令。某个版本才有的接口会有提示，没有提示的话表明是两个版本都有的接口，并且用法一样，只是内部实现可能不同。

所有 `optName` 都是短选项名或长选项名（不需要加上 `-` 或 `--`），也可以是两个都有（短选项名在前，长选项名在后，用 `,` 分隔），例如 `o`, `output`, `o,output`. 所有 `description` 都是用于在帮助中显示。
...
因为在预处理的时候，库并不知道选项的具体信息，当遇到 `-n123` 这种参数时，不知道是 `-n -1 -2 -3`, `-n 123`, `-n -1 23` 还是 `-n -1 -2 3`。默认是把它们都当成短标志选项，即 `-n -1 -2 -3`。如果你想支持带值短选项后面马上接它的值的话，你必须在预处理前告诉库，即调用 `setShortNonFlagOptsStr()`，它的参数是以字符串的形式书写的所有带值短选项。这样预处理时才能正确识别带值短选项和值。注意：只是传入**带值选短选项**，不是所有的短选项，用作标志的短选项不需要也不能传入。

这个缺点是这个库最不优雅的一点，这种独特的处理思路很难优雅地提供这个语法。好在不影响核心功能，也能通过脚本自动化地完成这种简单的修改。本项目提供了一个脚本示例，详见[示例](#-示例)章节。

如果你不想引入脚本，建议等当前阶段的工作结束后，例如要提交时，再更新一次字符串。因为不更新 `setShortNonFlagOptsStr()` 也能使用常规的、用空格分隔的语法 (`-n 123`)，所以不必添加一个选项就更新一次。在支持多光标的编辑器 / IDE 中通常可以用下面的 RegEx 选中所有带值短选项，然后构建出合适的字符串。
...
对于未知类型, `get<T>()` 会尝试使用解析到的字符串去初始化它。如果你的自定义类型可以用字符串初始化，那么理论上可以直接用它获取解析结果。但它无法提供校验，帮助和错误信息中不能提供相关信息。所以更推荐下面这种获取基本类型然后手动处理的方法。

---

**设置错误信息**

本例使用了 `get<std::string>()`, 使用其他基本类型的流程也一样。

先以基本类型形式获取选项的值，然后写你的解析和校验逻辑，在参数错误分支调用这个接口插入错误信息。
...
没定义 `ARGLITE_ENABLE_FORMATTER` 宏时，它们直接会返回传入字符串的视图。定义宏时，它们会返回一个格式化后的字符串，输出到文件或管道时不添加 ANSI 序列。

第二个参数是用于检测的输出流，函数会根据提供的流判断它是否输出到终端。 `bold()`, `boldUnderline()` 的第二个参数的默认值是 `std::cout`, 但错误信息输出到 `std::cerr`, 所以在这里使用它们时需要给第二个参数传入 `std::cerr`. 你也可以统一给第二个参数传入 `std::cerr`, 这样就不需要记它们的默认值。
...
具体用法参考这个[示例](examples/subcommand.cpp)，这个示例也演示了完整版的绝大部分功能。本节只作补充说明，你需要结合本节的内容和示例代码理解子命令功能的用法。
...
本库只支持一级子命令，不支持多级子命令。需要多级子命令的程序应该也不会要求命令行参数解析库能做得很轻量，不是本库的目标用户。不过本项目提供了一个示例，展示了 ArgLite 构建多级子命令程序的潜力，详见[示例](#-示例)章节。
...
检查该子命令是否被激活。因为注册选项时就会返回一个值，不管当前激活的命令是哪个，没激活时会返回类型的默认值。所以库提供了这个成员方法用于检查该命令是否激活，还提供了 `bool Parser::isMainCmdActive()` 用于检查主命令是否激活。
...
本章能让你快速了解各个示例的内容，并且能帮助你理解示例的用法或编写思路。

**[simple.cpp](./examples/simple.cpp)**

只使用了基本功能的示例，展示了精简版的用法。

**[subcommand.cpp](./examples/subcommand.cpp)**

使用了子命令功能，并且使用了几乎所有的完整版的功能，模仿了 Git 的参数解析，展示了如何使用 ArgLite 完成复杂的命令行参数解析。

**[option_grouping.cpp](./examples/option_grouping.cpp)**

展示如何使用选项分组功能，模仿了 ripgrep 的帮助信息。

**[simple_wrapped.cpp](./examples/simple_wrapped.cpp)** & **[subcommand_wrapped.cpp](./examples/subcommand_wrapped.cpp)**

前面的示例都只是展示了库的用法，并不适合作为实际应用的参考。这两个示例使用类包装参数，通过对象访问解析结果。并且利用 C++ 类的初始化顺序和单例模式，固化参数解析的工作流，保证各个接口的调用顺序符合库的要求，保证只解析一次。通过包装，添加参数和添加子命令变得更像写配置文件。

你只需复制模板代码，根据你的实际需要修改程序信息、参数、子命令即可，无需关心库的工作流程。有了这两个示例，你就能愉快地当一个 CV (Ctrl+C Ctrl+V) 程序员了。

**[update_short_opts.py](./examples/update_short_opts.py)**

这个脚本用于解决“需要更新 `setShortNonFlagOptsStr()`”这个痛点。

脚本使用 RegEx 查找和修改函数的参数字符串，适配了两个 wrapped 示例。如果你以它们为模板，只是修改为实际的参数，那么只需

- 修改脚本开头的 `DEFAULT_FILES` 列表为实际文件。
- 根据使用的版本进行修改：
  - 如果使用*精简版*，根据脚本中的提示，选择正确的 `MAIN_PARSER_GETTER_PATTERN`, 清空 `SUBCOMMAND_NAMES` 列表。
  - 如果使用*完整版*，不用修改任何 RegEx.
    - 如果*不*使用子命令，清空 `SUBCOMMAND_NAMES` 列表。
    - 如果使用子命令，修改 `SUBCOMMAND_NAMES` 为实际的子命令名字。注意 C++ 代码中子命令相关的命名应该遵循模板中的规则，否则可能匹配失败。

否则你可能需要修改相关 RegEx 以适配你的项目。

配置好脚本后不需要任何命令行参数即可使用，你也可通过传递位置参数处理指定文件。

因为脚本依赖已有的 `setShortNonFlagOptsStr()` 进行定位，所以你需要先写一句 `setShortNonFlagOptsStr("")`, 传入空字符串就行了。脚本不会猜要在哪里插入然后帮你插入，你必须自己写。

通过这个脚本, ArgLite 真正地做到了一条语句就对应一个选项，无需关心其他位置的代码。

**[nested_subcommand.cpp](./examples/nested_subcommand.cpp)**

该示例只是作为实现多级子命令应用的演示，不建议使用 ArgLite 构建这类应用。因为 ArgLite 在设计时根本没考虑实现多级子命令，本示例也也只是证明可行性，没有经过可靠性测试。

实现的思路很简单: 既然能支持一级子命令，那么给一级子命令挂上子命令就能让树形结构有更多的层次。理论上可以这样一直添加更多两层的树，最终构建出复杂的树。当然手动构建一个复杂的树需要非常多的代码，很难维护，但真实的应用不会有很复杂的子命令结构，所以这种方案没有大问题。

因为 ArgLite 设计为一次性解析，所以需要在调用 ArgLite 的接口时，要先手动判断当前激活的命令，然后在这个分支调用 ArgLite 的接口，并在这个分支中返回。不能先执行主命令的解析逻辑，然后用 `subcommand.isActive()` 检查，再去执行子命令的解析逻辑。多次解析不但效率更低，也很容易得到错误的结果。
