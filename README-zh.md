...
和主流库相比，本库的一大优势是可以做到一条语句完成一个参数，多加一个参数只要多加一条语句。 CLI11 加一个参数要加一条声明语句，提供用于后续绑定的变量，然后添加一条注册语句。 cxxopts 则是要一条注册语句，解析后要一条取值语句。其他库大部分都是这两种情况，都有同一个参数的代码不够集中的问题，只是具体的操作不同。
